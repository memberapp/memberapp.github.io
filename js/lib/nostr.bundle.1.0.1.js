...
zoo`.split("\n");
  ...
  function verifySignature(event) {
    return schnorr.verifySync(event.sig, event.id, event.pubkey);
  }
  ...
  var encode = function(arraybuffer) {
    var bytes2 = new Uint8Array(arraybuffer), i, len = bytes2.length, base642 = "";
    for (i = 0; i < len; i += 3) {
      base642 += chars[bytes2[i] >> 2];
      base642 += chars[(bytes2[i] & 3) << 4 | bytes2[i + 1] >> 4];
      base642 += chars[(bytes2[i + 1] & 15) << 2 | bytes2[i + 2] >> 6];
      base642 += chars[bytes2[i + 2] & 63];
    }
    if (len % 3 === 2) {
    ...
    deriveChild(index) {
      ...
      try {
        if (this.privateKey) {
          ...
          opt.publicKey = Point.fromHex(this.pubKey).add(Point.fromPrivateKey(childTweak)).toRawBytes(true);
        }
  ...
  function decode2(nip19) {
    let { prefix, words } = import_bech32.bech32.decode(nip19, 1e3);
    let data = new Uint8Array(import_bech32.bech32.fromWords(words));
    if (prefix === "nprofile") {
  ...
  function encodeBytes(prefix, hex2) {
    ...
    let words = import_bech32.bech32.toWords(data);
    return import_bech32.bech32.encode(prefix, words, 1e3);
  }
  function nprofileEncode(profile) {
    ...
    let words = import_bech32.bech32.toWords(data);
    return import_bech32.bech32.encode("nprofile", words, 1e3);
  }
  function neventEncode(event) {
    ...
    let words = import_bech32.bech32.toWords(data);
    return import_bech32.bech32.encode("nevent", words, 1e3);
  }
