...
async function userSearchChanged(searchbox, targetelement) {
  ...
  document.getElementById(targetelement).innerHTML = document.getElementById(
    "loading"
  ).innerHTML;
  await sleep(500);
  ...
  getJSON(
    dropdowns.contentserver +
      "?action=usersearch&address=" +
      pubkey +
      "&searchterm=" +
      encodeURIComponent(searchtermHOSTILE)
  ).then(
    function (data) {
      var test = data;
      ...
      for (var i = 0; i < data.length; i++) {
        contents =
          ...
          userHTML(
            data[i].address,
            data[i].name,
            i + searchbox + data[i].address,
            data[i].rating,
            16
          ) +
          sendEncryptedMessageHTML(
            data[i].address,
            data[i].name,
            data[i].publickey
          ) +
