...
function userHTML(address, name, ratingID, ratingRawScore, ratingStarSize) {
    ...
    var ret = `<div class="membercontainer"><a href="#member?qaddress=` + san(address) + `" onclick="showMember('` + san(address) + `')" class="hnuser"><svg class="jdenticon" width="20" height="20" data-jdenticon-value="` + unicodeEscape(name) + `"></svg>` + ds(name) + `</a> `;
    if (ratingStarSize > 0) {
        ret += `<div class="starrating"><div data-ratingsize="` + Number(ratingStarSize) + `" data-ratingaddress="` + san(address) + `" data-ratingraw="` + Number(ratingRawScore) + `" id="rating` + ratingID + `"></div></div></div>`;
    }
    return ret;
}
